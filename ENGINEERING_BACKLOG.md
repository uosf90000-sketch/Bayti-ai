...
| **Bayti Catalog Builder v1** — `packages/catalog-builder` + `packages/catalog-data` (402 سجل خام → 388 منتجًا/394 عرضًا) + `CatalogRepository` خلف `/api/catalog/*` | ✅ **المرحلة 1 معتمدة ومطبَّقة**: قسم "الكتالوج الحقيقي" في شاشة Shopping يعرض 61 منتجًا اجتازوا حد الجودة (60+) فقط، حد الجودة مطبَّق داخل `CatalogRepository` نفسها لا الشاشة. ✅ **المرحلة 2 — البنية جاهزة**: `review-queue.csv` جدول تعبئة يدوية جاهز + إعادة الحساب مُثبَتة (اندماج تلقائي رفع KIVIK لأعلى الحد). ⚠ **إكمال الـ 331 عرضًا المتبقية محجوب**: IKEA وPan Home يرجعان 403 لأي جلب آلي، ولا نتجاوز حماية المواقع (PRD §6) — بانتظار قرارك: إدخال يدوي بشري أم اتفاقية API رسمية. التقرير الكامل: `docs/CATALOG_INTEGRATION.md` |
