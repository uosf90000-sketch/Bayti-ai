...
# القسم 11 — Security, Infrastructure & Deployment

> قسم توحيدي: يجمع ويُقفل ما تناثر في المجمد (4.21، 6.26، 7.14، 9.12، 10.13، ووثيقة الأمن التأسيسية) ويضيف البنية والنشر. لا Features جديدة.

## 11.1 معمارية الأمان (الطبقات المجمدة — المرجع الواحد)

| الطبقة | المرجع المجمد | الخلاصة |
|--------|----------------|----------|
| بيانات المخططات | 4.21 | تشفير كامل، presigned ≤15د، عزل لكل مشروع، حذف مدقق بالبصمة |
| أمان الوكلاء وLLM | 6.26 + 9.12 | Context Contracts بلا PII، نصوص المستخدم/OCR/المنتجات بيانات غير موثوقة، schema-bound outputs، لا أدوات للمحادثة، tool allowlists، supply chain، secrets rotation 90 يومًا |
| التجارة | 7.14 | عزل شبكي كامل للـ ingestion، SSRF allowlists، تعقيم أوصاف المنتجات |
| المدفوعات | 10.13 | PCI صفر، server-authoritative pricing، HMAC webhooks |
| التطبيق | وثيقة الأمن (§9 القديمة) — تُعتمد فقرات OWASP/ASVS L2 والرفع الآمن (sandbox للـ CAD) كما هي | ✔ |

- **الهوية والوصول:** OTP سعودي + JWT قصير/refresh rotation؛ أدوار `user/pro/ops/admin`؛ Admin: SSO+MFA+audit؛ **العزل بالمستأجر على طبقة الـ repository** مع اختبارات تسرب آلية (لا اجتهاد endpoints).
- **الامتثال:** PDPL — سجل معالجة، أساس نظامي، إقامة البيانات في المنطقة، إجراءات الإبلاغ عن الحوادث؛ (SOC2 وشهادات B2B: Future Scope وفق خارطة الأمن المرحلية).

## 11.2 البنية التحتية (المرحلة A من خطة التوسع المجمدة — لا أكثر)

| المكوّن | القرار v1 |
|---------|-----------|
| السحابة | AWS me-south-1 (البحرين) — إقامة بيانات خليجية (الحزمة المجمدة) |
| الحوسبة | ECS/Fargate (لا Kubernetes قبل المرحلة B — قرار مجمد): API replicas + Workers |
| GPU | Serverless (Replicate/Modal) — صفر خمول، مزودان (failover 6.7) |
| البيانات | RDS PostgreSQL 16 + pgvector (primary + replica) · ElastiCache Redis · S3 + CloudFront |
| الشبكة | VPC خاصة: DB/Redis/Workers بلا IP عام · WAF على الحافة · bastion/SSO للوصول الإداري |
| Secrets | AWS Secrets Manager + الدوران الآلي (6.26) — **صفر أسرار في الريبو** (secret scanning في CI) |

## 11.3 البيئات

`dev` (compose محلي بأمر واحد — mocks للمزودات الخارجية) → `staging` (نسخة إنتاج مصغرة، بيانات اصطناعية + Golden Set، كل الاختبارات الثقيلة هنا) → `prod`. **قاعدة:** لا بيانات مستخدمين حقيقية خارج prod أبدًا.

## 11.4 CI/CD (بوابات الجودة — ترميز القواعد المجمدة)

```
PR ──► lint + typecheck + unit ──► contracts/schema compatibility (ADR-029)
    ──► rule-pack test_cases (100%) ──► AI evals عيّنية ──► security scans (SAST/deps/secrets)
    ──► مراجعة بشرية (+ بند connectors القانوني AC7-10) ──► merge
main ──► build صور موقعة ──► staging تلقائي ──► Golden Set الليلي + E2E الحية (فيلا النرجس/F2E-1)
     ──► prod بموافقة يدوية ──► rolling deploy + احتمال rollback فوري
```
- **أي أحمر = لا دمج** — بما فيها تراجع evals (بوابة 6.24: لا ترقية بتراجع هندسي).

## 11.5 استراتيجية النشر

Rolling مع health checks وrollback آلي عند تدهور المؤشرات · **Feature flags** لكل ميزة مستخدم جديدة (تفعيل تدريجي 10%→100%) · نسخ القواعد عبر **Shadow** (المسار المجمد 6.8) · هجرات DB متوافقة للأمام دائمًا (expand→migrate→contract) — لا هجرة كاسرة أثناء التشغيل.

## 11.6 النسخ الاحتياطي وDR

RDS PITR (احتفاظ 30 يومًا) + snapshot يومي معزول الحساب · S3 versioning + replication داخل المنطقة · **اختبار استعادة شهري مؤتمت** (يثبت RTO/RPO لا يفترضهما): RPO ≤ 15 دقيقة، RTO ≤ 4 ساعات (v1) · تمرين DR كامل قبل الإطلاق العام (بوابة G-5) · كتيبات incident response بأدوار مناوبة + قالب إبلاغ PDPL.

## 11.7 المراقبة والتشغيل

OpenTelemetry موحد (tracing يربط الطلب بكل وكلائه وتكاليفه) · Grafana dashboards لكل metrics الأقسام (4.23، 6.25، 7.13، 8.14، 9.13، 10.14) بمالك لكل لوحة · Sentry للأخطاء · إنذارات مصنفة P0–P2 بقنوات مناوبة · **runbook لكل إنذار P0/P1** (لا إنذار بلا إجراء مكتوب).

## 11.8 Acceptance Criteria

AC11-1: اختبار عزل المستأجرين الآلي أخضر دائمًا · AC11-2: صفر أسرار في الريبو (فحص تاريخي + مستمر) · AC11-3: استعادة شهرية ناجحة موثقة · AC11-4: كل P0/P1 له runbook · AC11-5: pentest خارجي مُغلق الثغرات الحرجة قبل الإطلاق · AC11-6: قياسات zero-trust الأساسية (لا خدمة تصل لما لا تحتاجه — IAM per-service) مُراجعة.

**Future Scope:** SOC2 Type I/II · bug bounty · multi-region فعلي · K8s (المرحلة B بعتباتها المجمدة).

---

# القسم 12 — UI/UX System & Final Acceptance

> القسم الأخير. يحوّل [WOW_MOMENTS](WOW_MOMENTS.md) + شاشات القسم 3 إلى نظام تصميم وتنفيذ واجهة — ويُقفل الـ PRD بمعايير القبول النهائية. **يرسّخ قرار المؤسس: الواجهة تُبنى أولًا بعقود ثابتة وMocks.**

## 12.1 Design System — "Bayti Glass"

- **Tokens** (`packages/ui/tokens`): الألوان (كحلي ليلي، رملي، ذهب صحراوي مطفأ — لوحة WOW_MOMENTS)، الطباعة العربية (عناوين مجلة معمارية + أرقام مميزة)، المسافات، الظلال، الزجاجية (blur/opacity levels)، أنصاف الأقطار، طبقات z.
- **مكونات أساسية (RTL-native، جوال أولًا):** أزرار/حقول/بطاقات زجاجية · بطاقة وكيل (S8) · بطاقة عنصر تصميم (مواصفة+مبرر+منتج) · مبدّل النسخ الثلاث · شريط ميزانية حي · slider قبل/بعد · بطاقة توضيح (خيارات بآثارها) · عارض المخطط SVG التفاعلي · حاوية العارض 3D · فقاعات المحادثة بحالاتها · حالات فارغة/فشل/تحميل الموحدة (3.2).
- **Motion presets** مقننة من مبادئ WOW الست (durations/easings كـ tokens — لا أرقام حرة في الكود) + `prefers-reduced-motion` بديلًا كاملًا.
- **Storybook** لكل مكوّن بحالاته (RTL/جوال/داكن) — بوابة مراجعة بصرية لكل PR واجهة.

## 12.2 معمارية الواجهة (Contract-First + Mock-First — قرار مؤسس)

1. **العقود ثابتة قبل أي شاشة:** كل استدعاء عبر `packages/contracts` (Zod من الأقسام المعتمدة) — الواجهة لا تعرف شكل بيانات خارج العقود.
2. **Mock Service Layer** (MSW) يطبّق العقود نفسها بسيناريوهات واقعية (مشروع فيلا النرجس كاملًا كبيانات mock مرجعية: تحليل، مجلس حي، نتائج، تسوق، تعديلات) — **الواجهة تُبنى وتُختبر كاملة قبل جاهزية الخدمات.**
3. **الاستبدال التدريجي:** خدمة حقيقية تجهز → يُقلب flag لها فقط — **صفر تغيير في كود الشاشات** (العقد هو الضمان). ADR-032 يتحقق عمليًا هنا.
4. Next.js App Router + TanStack Query + SSE موحد؛ كل حالة خادم عبر العقود؛ لا منطق مجال في الواجهة (ADR-032).

## 12.3 الشاشات (تنفيذ القسم 3 كما اعتُمد)

خريطة S0–S17 ومعايير قبول كل شاشة **هي المرجع دون إعادة فتح** — هذا القسم يضيف فقط: ترتيب بناء الشاشات في الـ Sprints (أولوية المؤسس: مشروع جديد، دخول، لوحة مشاريع، رفع، شاشة المجلس أولًا) + ربط كل شاشة بمكوناتها من 12.1 وبحالة الـ mock الخاصة بها.

## 12.4 الوصولية والتعريب

WCAG AA (3.2 المجمدة): تباين، أهداف لمس ≥44px، قارئ شاشة للمسار الحرج، تنقل لوحة مفاتيح · `ar-SA` أولًا وبنية `en` جاهزة (next-intl) · كل النصوص من ملفات رسائل — صفر نصوص في الكود · أرقام/عملة/تواريخ بالتنسيق المحلي الموحد (3.2).

## 12.5 أداء الواجهة

LCP ≤ 2.5s (تسويقية) / ≤ 3s (تطبيق أول تحميل) على 4G · route-based code splitting + lazy للعارض 3D · ميزانية JS للتحميل الأول ≤ 300KB مضغوطة (بلا 3D) · صور AVIF/WebP عبر CDN · Web Vitals telemetry حية.

## 12.6 Final Acceptance — بوابات قبول الـ PRD كاملًا (تجميع نهائي)

| البوابة | المصدر المعتمد |
|---------|-----------------|
| دقة التحليل والتوأم | AC-1…AC-10 (§4.15) |
| الاستبيان والذوق | AC5-1…7 |
| المجلس والدمج | §6.28 (الاثنا عشر + E2E الثلاث + F2E-1) |
| التجارة | AC7-1…10 |
| المخرجات | AC8-1…7 |
| المحادثة | AC9-1…8 |
| الفوترة | AC10-1…8 |
| الأمن والتشغيل | AC11-1…6 |
| تجربة الاستخدام | مصفوفة §3.4 (10 مستخدمين حقيقيين، ≥8 بلا مساعدة) + WOW KPIs (§WOW-6) |
| الإطلاق العام | بوابة G-5 (Bootstrap) شاملة pentest وDR |

- **إعلان اكتمال الوثائق:** باعتماد هذا القسم تتحقق بوابة **"Documentation Complete — Go to Build"** (Bootstrap بند 2) — مع نقل TD-01/02/08 إلى الـ Engineering Backlog كمهام تنفيذ لا وثائق.

## 12.7 Acceptance Criteria (للقسم ذاته)

AC12-1: كل مكوّن في Storybook بحالاته الثلاث (RTL/جوال/تحميل-فشل-فارغ) · AC12-2: الرحلة الحرجة كاملة تعمل على mocks قبل أي خدمة حقيقية · AC12-3: استبدال mock بخدمة حقيقية دون تعديل كود شاشة (يُختبر على أول خدمتين) · AC12-4: صفر نصوص صلبة خارج ملفات الرسائل · AC12-5: ميزانيات 12.5 مقاسة في CI (Lighthouse budget).

**Future Scope:** Design system عام للنشر · ثيمات · تطبيقات native (v2 حسب الخارطة المجمدة).

---

## 🏁 خاتمة الـ PRD

**باعتماد القسمين 11 و12 يكتمل هذا المستند (الأقسام 1–12) وتتحقق بوابة "Documentation Complete — Go to Build".** لا وثائق جديدة بعد هذه النقطة (قرار مؤسس) — التالي: Engineering Backlog → Repository Structure → Sprint 0 → Sprint 1 (الواجهة الفاخرة أولًا فوق Mocks بعقود ثابتة) → استبدال تدريجي بالخدمات الحقيقية. **الهدف: أول نسخة تعمل من Bayti AI على الشاشة.**
