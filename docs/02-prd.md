...
| 4 | المواصفة الهندسية: رفع المخطط وتحليله وتحويله إلى Digital Twin | ✅ **معتمد** |
| 5 | المتطلبات الوظيفية: الاستبيان واختيار النمط | 🔍 **جاهز للمراجعة** |
...
# القسم 5 — المتطلبات الوظيفية: الاستبيان واختيار النمط

> **طبيعة القسم:** متطلبات وظيفية (Functional Requirements) لمرحلتي S6 (الاستبيان) وS7 (النمط والميزانية) — المرحلة التي تحوّل «مخططًا مفهومًا» (توأم رقمي معتمد من القسم 4) إلى «تكليف تصميم كامل» (Design Brief) يستلمه مجلس الوكلاء (القسم 6). كل متطلب مرقّم `FR-5.x` ليُتتبع في الاختبارات.

## 5.0 موقع المرحلة في النظام

```
Twin معتمد (القسم 4) ──► S6 الاستبيان ──► S7 النمط والميزانية ──► DesignBrief v1 ──► مجلس الوكلاء (القسم 6)
        حالة المشروع:  needs_review → intake → generating
```

- **المخرج الوحيد للمرحلة:** كيان `DesignBrief` (5.8) — عقد بيانات مفحوص بـ JSON Schema يجمع: إجابات الاستبيان + النمط (عام + استثناءات) + الميزانية وتوزيعها المبدئي + مقتطف ملف الذوق (P5) وذاكرة المشروع (P15).
- **قاعدة حاكمة (P12):** كل منطق هذه المرحلة (شروط الأسئلة الديناميكية، فحص تعارض الميزانية، التوزيع المبدئي) **قواعد حتمية** — لا LLM في أي قرار من قرارات المرحلة. الاستثناء الوحيد: ترشيح النمط من Style Quiz (نموذج تفضيل بصري) وهو **اقتراح** يقرره المستخدم دائمًا.
- واجهات المرحلة معرفة مسبقًا في وثيقة API: `PUT /projects/:id/intake` و`PUT /projects/:id/style` — هذا القسم يحدد حمولتيهما وسلوكهما.

## 5.1 الاستبيان الأساسي (الأسئلة السبعة)

**FR-5.1 —** يعرض النظام سبعة أسئلة أساسية، سؤالًا في خطوة، بالترتيب والمواصفة التالية:

| # | السؤال | نوع الإدخال | التحقق | إلزامي؟ | القيمة الافتراضية عند التخطي | «لماذا نسأل» (يظهر بسطر واحد) |
|---|--------|-------------|--------|---------|------------------------------|-------------------------------|
| Q1 | المدينة | قائمة مدن سعودية + بحث | مدينة معتمدة في جدول المدن | ✅ | — | «المناخ يحدد التكييف، والمدينة تحدد أسعار السوق» |
| Q2 | الميزانية الإجمالية | شريط تمرير (20k–1M ريال، خطوة 5k) + إدخال يدوي | رقم ≥ 20,000 ريال | ✅ | — | «كل اقتراحاتنا ستُقاس عليها — بصدق» |
| Q3 | أفراد الأسرة | عدّاد (1–20) | عدد صحيح | ⬜ | 4 (معلَنة للمستخدم) | «عدد المقاعد والأسرّة وأماكن التخزين» |
| Q4 | أطفال + أعمارهم | نعم/لا → شرائح أعمار (0–2، 3–6، 7–12، 13+) متعددة | إن «نعم» فشريحة واحدة على الأقل | ⬜ | لا يوجد | «زوايا آمنة، خامات قابلة للتنظيف، غرف تناسب الأعمار» |
| Q5 | كبار سن مقيمون | نعم/لا | — | ⬜ | لا | «سلامة الحركة والإضاءة وسهولة الوصول» |
| Q6 | حيوانات أليفة | نعم/لا → نوعها (قطط/كلاب/طيور/أخرى) | — | ⬜ | لا | «خامات مقاومة للخدش وأماكن مخصصة» |
| Q7 | أولويات الإنفاق | ترتيب/اختيار حتى 3 من: المجلس والضيافة، غرف النوم، المطبخ، المعيشة، الخارجي | ≤ 3 اختيارات | ⬜ | توزيع متوازن (معلَن) | «نضع ميزانيتك حيث يهمك أكثر» |

**FR-5.2 —** التخطي مسموح لكل سؤال غير إلزامي، وعند التخطي تُعرض القيمة الافتراضية **قبل** المتابعة («سنفترض: أسرة من 4») — لا افتراض صامت (روح P9).
**FR-5.3 —** كل إجابة تُحفظ فورًا (autosave لكل خطوة) — مغادرة الشاشة والعودة تستأنف من آخر سؤال، عبر أي جهاز.
**FR-5.4 —** إجابات Q3–Q6 تُخزَّن في كيان `intakes` (الحقول المعرفة في وثيقة قاعدة البيانات)، والمدينة والميزانية على مستوى المشروع، وأي إجابات ديناميكية في `extra_answers` (JSONB) بمفاتيح مسماة ثابتة.

## 5.2 الأسئلة الديناميكية (حسب المخطط — حدّها الأقصى سؤالان في الـ MVP)

**FR-5.5 —** بعد الأسئلة السبعة، يفحص **محرك قواعد حتمي** التوأم الرقمي ويولّد أسئلة إضافية من كتالوج مُدار كملف بيانات (`dynamic_questions.yaml`)، بقاعدة تشغيل صريحة لكل سؤال:

| المشغّل (شرط على الـ Twin) | السؤال | أثره على الوكلاء |
|-----------------------------|--------|-------------------|
| غرفة `type=pool` أو مسبح مكشوف | «عندكم أطفال — نضيف حماية للمسبح ضمن التصميم؟» (يظهر فقط مع Q4=نعم) | Landscape + قواعد سلامة P3 |
| غرفة `type=annex` (ملحق) | «الملحق: نؤثثه بالكامل أم نتركه خارج المشروع؟» | نطاق عمل المجلس والتكلفة |
| غرفة `type=driver_room / maid_room` | «غرفة السائق/الخادمة داخل نطاق التأثيث؟» | نطاق العمل |
| مساحة خارجية > 30م² | «الحوش: جلسة خارجية، حديقة، أم الحد الأدنى؟» | Landscape |
| طابقان+ وQ5=نعم (كبار سن) | «هل يحتاج كبير السن غرفة نوم في الدور الأرضي؟» | Interior Designer (توزيع الغرف) |

**FR-5.6 —** عند تحقق أكثر من مشغّلين، تُختار **أعلى أولويتين أثرًا** (ترتيب الأولوية حقل في الكتالوج) — لا يتجاوز الاستبيان 9 أسئلة أبدًا في الـ MVP (قرار 13-mvp).
**FR-5.7 —** كل سؤال ديناميكي يذكر سببه من مخطط المستخدم نفسه («لاحظنا مسبحًا في مخططك») — يعزز إحساس «النظام فهم بيتي» (لحظة الحقيقة م3).

## 5.3 تكامل الذاكرة: لا يُسأل المستخدم عن الشيء نفسه مرتين (P15 + P5)

**FR-5.8 —** قبل عرض أي سؤال، يمر بفحص «هل أجاب عنه سابقًا؟»:
- **من مشروع سابق (ملف الذوق P5):** تُعبَّأ الإجابة مسبقًا مع شارة مصدر («من مشروعك السابق — عدّلها إن تغيّر شيء») — تُعرض ولا تُتخطى، والتأكيد بضغطة واحدة.
- **من نفس المشروع (P15):** السؤال لا يظهر إطلاقًا (عاد للاستبيان بعد إكماله؟ يرى إجاباته للتعديل لا أسئلة فارغة).
**FR-5.9 —** تعديل إجابة بعد توليد التصميم (العودة من S9 مثلًا) يمر بمسار الإبطال الانتقائي (P10): تُحدَّد الوكلاء المتأثرون بالإجابة المتغيرة فقط ويُعاد تشغيلهم، وينشأ إصدار جديد (P7)، ويُسجَّل التغيير في الـ Audit Log (P14).

## 5.4 كتالوج الأنماط الثمانية

**FR-5.10 —** الأنماط الثمانية للـ MVP (كتالوج مُدار كبيانات — `styles.yaml` — الإضافة/التعديل قرار منتج لا نشر كود):

| # | النمط | الاسم المعروض | جمهوره الأساسي (تقريبي) |
|---|-------|----------------|--------------------------|
| 1 | `modern` | مودرن | أوسع الشرائح |
| 2 | `new_classic` | نيو كلاسيك | Persona 1 — المجالس خصوصًا |
| 3 | `classic_luxury` | كلاسيك فاخر | فلل كبيرة، النسخة الفاخرة |
| 4 | `contemporary_gulf` | خليجي معاصر (لمسة تراثية حديثة) | تمايزنا المحلي — لا يقدمه منافس عالمي |
| 5 | `minimalist` | مينيمال | Persona 2 |
| 6 | `scandinavian` | إسكندنافي | Persona 2 |
| 7 | `japandi` | جاباندي | Persona 2، ميزانيات متوسطة+ |
| 8 | `boho` | بوهو | شريحة أصغر — تقاس وتُستبدل إن لم تُستخدم |

**FR-5.11 —** كل نمط في الكتالوج يحمل: 4–6 صور مرجعية **لغرف سعودية/خليجية حقيقية النِّسب** (مجلس، معيشة، نوم — لا صور غربية عامة)، ووصفًا بسطرين بالعربية، و**بصمة نمط** تُمرَّر للوكلاء: لوحات ألوان، خامات مميزة، مستوى الزخرفة، وموجهات التوليد البصري.
**FR-5.12 —** بطاقة النمط تعرض مؤشر ملاءمة للميزانية (£/££/£££ بمنطق حتمي: متوسط كلفة النمط للمتر المربع من قاعدة الأسعار مقابل ميزانية/مساحة المستخدم) — الصدق قبل الاختيار لا بعده.

## 5.5 Style Quiz البصري (اختياري)

**FR-5.13 —** بديل للاختيار المباشر: 10 صور متتالية (أعجبني/لم يعجبني)، مختارة حتميًا من مجموعة موسومة الأبعاد (نمط، لوحة ألوان، كثافة زخرفة، خامات) بحيث تفصل بين الأنماط الثمانية بأقصى قدرة تمييز، مع تنويع نوع الغرفة.
**FR-5.14 —** الناتج ترشيح معاير (ADR-4.4 نفسها تنطبق: score خام → معايرة على بيانات اختبار موسومة):

| ثقة الترشيح (معايرة) | السلوك |
|----------------------|--------|
| ≥ 0.75 | نمط واحد مُرشَّح: «ذوقك أقرب إلى الجاباندي — بنسبة تطابق عالية» + عرض بطاقته للتأكيد |
| 0.50–0.75 | أفضل نمطين جنبًا إلى جنب: «ذوقك بين المودرن والإسكندنافي — اختر» |
| < 0.50 | لا ترشيح مُلزم: «ذوقك متنوع!» + عرض الأنماط الثمانية كاملة (النظام لا يخمّن — P9) |

**FR-5.15 —** القرار النهائي للمستخدم دائمًا: الترشيح قابل للتجاوز بضغطة، وإجابات الـ Quiz العشر تُخزَّن كاملة (بيانات تفضيل — G6) وتغذي ملف الذوق (P5) بوسم مصدر `style_quiz`.

## 5.6 النمط العام واستثناءات الغرف

**FR-5.16 —** يختار المستخدم نمطًا عامًا واحدًا، وله استثناء **حتى غرفتين** بنمط مختلف في الـ MVP (قرار 13-mvp) — الحالة الشائعة المستهدفة: «المجلس نيو كلاسيك والباقي مودرن».
**FR-5.17 —** الاستثناء يُخزَّن على مستوى الغرفة (`style_override` في كيان الغرفة) ويُمرَّر للوكلاء ضمن سياق الغرفة؛ ويتولى Interior Designer AI **الانسجام الانتقالي** بين الغرف المتجاورة مختلفة النمط (ممرات، أرضيات مشتركة) — متطلب صريح في prompt الوكيل يُختبر في الـ evals (القسم 6).
**FR-5.18 —** محاولة إضافة استثناء ثالث تُرفض برسالة صادقة تشرح الحد وتعد به في نسخة قادمة — لا فشل صامت.

## 5.7 الميزانية: العرض الصادق والتوزيع المبدئي

**FR-5.19 —** بعد اختيار النمط تُعرض الميزانية مقابل **توزيع مبدئي تقديري** بالأقسام (أثاث/تشطيب/مطبخ/إنارة وكهرباء/خارجي)، محسوبًا حتميًا من: مساحات الغرف الفعلية (من الـ Twin) × متوسطات كلفة النمط المختار للمتر (قاعدة أسعار السوق المؤرخة) × أوزان أولويات Q7 — ويُعلَّم بوضوح «تقديري — يتأكد بعد التصميم» (قاعدة S12: التقديري والحقيقي لا يختلطان).
**FR-5.20 — فحص التعارض (حتمي، قبل إطلاق المجلس):**

| الحالة (نسبة الميزانية إلى الحد الأدنى المقدَّر للنمط والمساحة) | السلوك |
|--------------------------------------------------------------|--------|
| ≥ 1.0 | عبور صامت |
| 0.7–1.0 | تنبيه أصفر: «ميزانيتك مشدودة لهذا النمط — سنقدّم أفضل الممكن وقد نقترح تنازلات مدروسة» |
| < 0.7 | التنبيه الصادق الكامل (معيار S7-③): «هذا النمط بهذه الميزانية سيكون تحديًا — سنريك أفضل الممكن + نسخة أعلى للمقارنة» + اقتراح نمطين بديلين أوفر (بحكم FR-5.12) |

**FR-5.21 —** لا يُمنع المستخدم من المتابعة في أي حالة (الميزانية طموحه لا قيدنا) — لكن التنبيه يُسجَّل في `DesignBrief` ليعرف Cost Engineer AI أن الالتزام بالميزانية أولوية قصوى في هذا المشروع.

## 5.8 المخرج: عقد بيانات `DesignBrief v1`

```jsonc
{
  "brief_version": "1.0",
  "project_id": "prj_...",
  "twin_revision": 3,                      // مربوط بإصدار توأم محدد — تغيّر التوأم يبطل الـ brief
  "intake": {
    "city": "riyadh",
    "budget_total_sar": 250000,
    "family_size": 5,
    "children": { "present": true, "age_bands": ["3-6", "7-12"] },
    "elderly": false, "pets": { "present": false },
    "priorities": ["majlis_hospitality", "bedrooms", "kitchen"],
    "extra_answers": { "annex_in_scope": true, "pool_child_safety": true },
    "answer_sources": { "family_size": "user", "city": "taste_profile_prefill" }  // P15/P5 شفافية المصدر
  },
  "style": {
    "primary": "modern",
    "overrides": [{ "room_id": "room_a1f4", "style": "new_classic" }],
    "selection_method": "quiz",            // quiz | manual
    "quiz_confidence": 0.82                // إن وجد — معاير (P9)
  },
  "budget_allocation_estimate": [ { "section": "furniture", "sar": 95000, "basis": "market_prices_2026-07" } ],
  "budget_tension": "none",                // none | tight | challenging (FR-5.20)
  "audit": [ { "at": "...", "actor": "user", "action": "brief_confirmed" } ]
}
```

- **FR-5.22 —** الـ Brief مفحوص بـ JSON Schema (`packages/design-schema`) — فشل الفحص يمنع إطلاق المجلس (نفس قاعدة AC-8).
- **FR-5.23 —** شاشة تأكيد أخيرة تلخص الـ Brief بالعربية بجمل («أسرة من 5 في الرياض، ميزانية 250 ألف ريال، مودرن مع مجلس نيو كلاسيك، الملحق داخل النطاق») + زر «شغّل المجلس» — آخر نقطة تعديل رخيصة قبل التوليد المكلف.

## 5.9 حالات الحواف

| الحالة | السلوك |
|--------|--------|
| مستخدم يغلق التطبيق منتصف الاستبيان | استئناف من آخر سؤال (FR-5.3) — يُذكَّر بإشعار بعد 24 ساعة |
| ميزانية غير واقعية إطلاقًا (< 20k لمنزل كامل) | يُقترح تحويل المشروع إلى «غرفة واحدة» (باقة Free) بدل رفض جاف |
| مخطط بلا مساحة خارجية + أولوية Q7 = «الخارجي» | الخيار لا يُعرض أصلًا (الأولويات تُبنى من الـ Twin — لا خيار بلا موضوع) |
| صور الـ Quiz لا تُحمَّل (شبكة ضعيفة) | سقوط تلقائي لبطاقات الأنماط النصية/المصغرة — لا شاشة معطلة |
| المستخدم عدّل التوأم (S5) بعد إكمال الاستبيان | تُعاد فقط الأسئلة الديناميكية المتأثرة (ظهر ملحق جديد؟) — الإجابات السبع لا تُمس (P10) |

## 5.10 Acceptance Criteria — القسم 5

| # | المعيار | العتبة |
|---|---------|--------|
| AC-5.1 | إكمال الأسئلة السبعة (P95، جوال) | ≤ 60 ثانية |
| AC-5.2 | Style Quiz كاملًا (P95) | ≤ 90 ثانية |
| AC-5.3 | اختيار نمط مباشر بلا Quiz (P95) | ≤ 45 ثانية |
| AC-5.4 | معدل إكمال المرحلة (S6 دخول → Brief مؤكد) | ≥ 85% من مستخدمي المرحلة |
| AC-5.5 | كل سؤال يعرض «لماذا نسأل» + كل تخطٍّ يعرض افتراضه المعلن | 100% |
| AC-5.6 | لا سؤال يُعرض على مستخدم أجاب عنه سابقًا (P15) — يُختبر آليًا | 100% |
| AC-5.7 | دقة ترشيح الـ Quiz: المستخدم يُبقي النمط المرشح (لا يبدله فورًا) | ≥ 70% من ترشيحات الفئة ≥ 0.75 |
| AC-5.8 | كل `DesignBrief` مطابق للـ Schema قبل إطلاق المجلس | 100% |
| AC-5.9 | فحص تعارض الميزانية يعمل قبل التوليد ونتيجته مسجلة في الـ Brief | 100% |
| AC-5.10 | كل قيم الـ Brief قابلة التتبع لمصدرها (إجابة/افتراض معلن/ذاكرة) في الـ Audit Log | 100% |

> **بوابة القسم:** الـ Brief هو العقد الكامل الذي يعمل عليه المجلس — أي معلومة يحتاجها وكيل وليست فيه تُضاف هنا (سؤالًا أو استنتاجًا حتميًا من الـ Twin)، **ولا يُسمح لأي وكيل بسؤال المستخدم مباشرة** أثناء التوليد.
