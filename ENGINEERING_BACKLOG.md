...
| **Bayti Catalog Builder v1** — دمج `packages/catalog-builder` + دمج 402 سجل خام (Batch 01–14 + library-v1) إلى 388 منتجًا كنسيًا/394 عرضًا في `packages/catalog-data/generated` + `CatalogRepository` (JSON/Supabase) خلف `/api/catalog/*` | ✅ **الأداة والبيانات جاهزة، صفر رفض، صفر بيانات مختلقة** — ⚠ **84% من العروض (331/394) بجودة <60 لنقص السعر/المقاسات في Batch 04–14 بالكامل** (محجوبة عن العرض حتى تحقّق حي) — 🔶 **شاشة Shopping غير موصولة بعد بالكتالوج الحقيقي عمدًا** (قرار منتج يحتاج تأكيدك). التقرير الكامل: `docs/CATALOG_INTEGRATION.md` |
